...
| Open | **A: 0 P0 · 23 P1 · 38 P2 · 20 P3 — D: 1 P0 · 2 P1 · 1 P2 · 3 P3** (re-derived from the body by `check-register-consistency`, not hand-counted) |
| Counts | A-P0: — · A-P1: A179 A151 A54 A18 A19 A20 A50 A24 A129 A158 A160 A161 A162 A163 A164 A211 A240 A242 A254 A265 A266 A267 A268 · A-P2: A189 A188 A182 A168 A22 A23 A53 A141 A146 A159 A157 A197 A207 A209 A212 A217 A224 A229 A231 A241 A244 A245 A246 A248 A249 A250 A252 A253 A255 A269 · A-P3: A186 A13 A139 A148 A204 A216 A230 A232 A233 A234 A237 A243 A247 A251 A257 A258 A261 A262 A263 — D-P0: D1 · D-P1: D3 D4 · D-P2: D18 · D-P3: D9 D10 D17 |
...
**The one caveat (rule 16):** `order:create` is validated against
`createLocalOrder`'s real nested shape but is listed in `NEEDS_LIVE_TEST` — it is
the primary sale path and could not be rung on the bench, so confirm it on ONE
real order on a dev-flavour till before production. It is validated, not
blind-trusted; it simply carries a one-order confirmation the others don't.
...
gate source guards); 96/96 offline suites. NOT verified here: the one live sale
above, and the desktop runtime suite (`npm run test:desktop`, target-machine).
Delivery: `docs/MANIFEST-2026-09-09-b.md`.
...
same mint function, but the block that renders the returned code (`enrolResult`: Business ID +
per-till codes + Copy + expiry) existed ONLY on the Overview tab. Clicking Enrol till on the
Branches tab (the natural home, alongside Tills →/Activate/Revoke) showed the count prompt, minted
the code server-side, then rendered nothing — and because codes are single-use, each click silently
...
### A204 · P3 · OPEN · Transfer Cancel uses a native window.confirm and sends no cancellation reason (server requires one)
...
### A211 · P1 · FIX BUILT 2026-09-05 · Permission catalogue missing 7 keys on dump-seeded DBs — customer/receiving/ingredient/financial routes silently owner-only
...
### A212 · P2 · FIX BUILT 2026-09-05 · Manager Printers tab gated on owner-only settings.manage — never visible to any manager
...
### A217 · P2 · FIX BUILT 2026-09-05 · Reports → Shifts shows "Unknown" for a shift Overview attributes correctly
...
### A224 · P2 · FIX BUILT 2026-09-05 · Transfer received quantity + receipt note stored but invisible in the UI (A221 follow-up)
...
### A229 · P2 · FIX BUILT 2026-09-05 · GRN receive has no note field — a short delivery can't record why (owner GRN receive had one; manager's didn't)
...
### A230 · P3 · FIX BUILT 2026-09-05 · GRN reprint line shows "Invalid Date" (fmtDate broke on ISO timestamps)
...
### A231 · P2 · FIX BUILT 2026-09-05 · No printable end-of-day Z report (desktop has one; needed to run a shop)
...
### A232 · P3 · FIX BUILT 2026-09-05 · Printed documents have no company logo (owner request)
...
### A233 · P3 · FIX BUILT 2026-09-05 · No stock-take count sheet to print (owner request)
...
### A234 · P3 · FIX BUILT 2026-09-05 · Doc-spec builders duplicated across four files (drift risk)
...
### A235 · P2 · FIX BUILT 2026-09-05 · Web silent receipt printing never worked (dashboard↔bridge contract mismatch)
...
### A237 · P3 · OPEN · Print-bridge exe is 57 MB (Node runtime); a tiny (~5 MB) Go byte-forwarder is built but not yet wired
...
### A238 · P2 · FIX BUILT 2026-09-05 · Manager Printer Setup stuck on "Select a branch" (BranchContext is owner-only) — blocks printer pairing
...
### A244 · P2 · FIX BUILT 2026-09-07 · Test print dialed the printer NAME as a network host — "Send test receipt" (and station tests) failed with "no such host"
...
### A269 · P2 · FIX BUILT 2026-09-07 · Print Bill printed a full receipt with a throwaway number — now a marked proforma BILL
...
### A268 · P1 · FIX BUILT 2026-09-07 · Double printing — payment re-fired the kitchen after Send to Kitchen; Print Bill + payment print two receipts
...
### A267 · P1 · FIX BUILT 2026-09-07 · Every POS api call 401'd after the access token expired — the ROOT of the "business null" saga
...
### A266 · P1 · FIX BUILT 2026-09-07 · After payment the receipt was a blank white box and did not auto-print
...
### A265 · P1 · FIX BUILT 2026-09-07 · Charge crashed the web POS — receiptHeader was referenced but never declared
...
### A263 · P3 · FIX BUILT 2026-09-07 · Report period selector — Apply button redundant, presets double-highlighted, day not default
...
### A258 · P3 · FIX BUILT 2026-09-07 · Overview wasted space — Top Items + Payment Methods now side by side
...
### A259 · P2 · FIX BUILT 2026-09-07 · Reports showed "Unknown" cashier and "No shifts" despite a running shift
...
### A260 · P2 · FIX BUILT 2026-09-07 · Printed documents show "SwiftPOS" instead of the client's business name
...
### A261 · P3 · FIX BUILT 2026-09-07 · No "Reprint receipt" (duplicate) action on the Orders page
...
### A262 · P3 · FIX BUILT 2026-09-07 · No shift report under Shifts
...
### A257 · P3 · FIX BUILT 2026-09-07 · Validation errors showed "Validation failed" with no field; category placeholder was petrol-specific
...
### A256 · P2 · FIX BUILT 2026-09-07 · New default permissions never reach roles created before they existed (backfill)
...
### A255 · P2 · FIX BUILT 2026-09-07 · Receipt was missing branch + owner header/footer; Print Bill opened the cash drawer
...
### A254 · P1 · FIX BUILT 2026-09-07 · Web tickets never cut, had no bottom margin, and Master KOT printed as a 2nd dispatch (missing kitchen)
...
### A253 · P2 · FIX BUILT 2026-09-07 · Print parity Phase 4 — send-vs-pay timing split + single routed fan-out
...
### A252 · P2 · FIX BUILT 2026-09-07 · Print parity Phase 3b (Option B-engine) — web routes combo COMPONENTS by category via the shared engine
...
### A251 · P3 · FIX BUILT 2026-09-07 · Print parity Phase 2b — desktop adopts the shared routing module (duplicate copy removed)
...
### A250 · P2 · FIX BUILT 2026-09-07 · Print parity Phase 3 — kitchen exclusions now applied (drinks etc. dropped from kitchen tickets)
...
### A249 · P2 · FIX BUILT 2026-09-07 · Print parity Phase 2 — routing logic extracted to shared/ (one copy, DB-free), characterization-tested
...
### A248 · P2 · FIX BUILT 2026-09-07 · Print parity Phase 1 — web carries combo components + per-item category, so kitchen/dispatch tickets show the breakdown
...
### A247 · P3 · FIX BUILT 2026-09-07 · Print-Bill station order was undefined — should be kitchen → customer → dispatcher
...
### A246 · P2 · FIX BUILT 2026-09-07 · Restaurant "Print Bill" used a browser dialog + ad-hoc format + printed once — now silent, shared-format, fans to the 3 full-order stations
...
### A245 · P2 · FIX BUILT 2026-09-07 · Kill the pairing-token copy-paste — trust the dashboard origin, and survive Chrome's Local Network Access
...
### A240 · P1 · FIX BUILT 2026-09-07 · Print bridge was DNS-rebinding-exploitable + leaked printer names (loopback + token were not enough)
...
### A241 · P2 · FIX BUILT 2026-09-07 · Dashboard read VITE_PRINT_SERVER_URL despite the register claiming "hard-coded, no Vercel env" — a silent-misroute trap, and the test asserted the opposite of its own name
...
### A242 · P1 · FIX BUILT 2026-09-07 · "⚡ Print Server" kitchen tickets never printed — KOT path still spoke the old QZ contract the v4 bridge 400s (silent loss)
...
### A243 · P3 · FIX BUILT 2026-09-07 · Retire the dead A209/A235 print path + sweep printToQZ; honour copies; fix modifier receipt field
...
### A239 · P2 · FIX BUILT 2026-09-05 · Silent printing needs a small installer + actually-matching contracts (57 MB bridge, dashboard↔bridge mismatch)
...
### A209 · P2 · FIX BUILT 2026-09-04 · Web receipts print in a different format from the desktop (no shared render)
...
### A208 · P3 · FIX BUILT 2026-09-04 · Manager web portal has no menu view (read-only Menu tab)
...
(refunded)` reversal line, the intended accounting difference from a void (which flips to Voided).
But on Finance → Orders a refunded order is pixel-identical to a clean completed sale; the only
tell is the reversal line beneath it (e.g. ORD-MTKAOJNI-002269). At a glance, refunds are
invisible. **Fix direction:** keep the green "Completed" status and add an amber **"Refunded"**
badge beside it (plus a "Partially refunded" variant if partial refunds exist), reading distinctly
from "Voided" so the two money-reversal paths aren't confused. Do NOT relabel Completed → Refunded
— that would wrongly imply the sale didn't stand. Sibling of A193 (refund has no audit-log view):
...
defect: a voided order is cancelled, so a still-live ticket means the kitchen keeps cooking food
nobody is paying for — direct product loss. For a **refund** it is softer (food may already be
made) but a still-pending refunded ticket is likely wrong too. Root cause: the kitchen-ticket
lifecycle isn't linked to the order's void/refund transition — voiding an order doesn't pull or
cancel its ticket. **Fix direction:** on order void, cancel/remove (or clearly strike) the
associated kitchen ticket; for refunds, a small policy call — pull only if not yet prepared. Same
family as A3 but a distinct lifecycle bug (A3's deliver + advance criteria passed), so it carries
its own ID. Arguably P1 if voids are frequent, given it is direct product loss. Files: order
...
min-threshold slice (closed 2026-09-02 on prod). The remaining inventory write-endpoints from that
audit — stock-transfer approve/complete, and direct branch-stock set — exist on the server but
still have no dashboard UI caller, so they can't be exercised from the owner surface. Carried here
so the A144 close doesn't swallow the untested remainder. **Fix direction:** wire the two endpoints
...
`KDS tickets: unexpected response`, yet the display still shows the green "connected" dot and
"All clear — no pending tickets." A kitchen whose token expired or was never paired therefore
looks healthy while silently showing zero tickets as live orders pile up unseen — the failure
only surfaces when staff notice missing food. The status indicator keys off poll liveness, not
fetch success, so a 401 renders indistinguishably from a genuine empty queue. **Fix direction:**
treat a tickets-fetch 401/non-200 as an explicit un-paired / auth-failed state (red status + a
...
dialog states the reason is recorded against the authorizer's name the same way, but there is NO
equivalent Refund Log surface in the owner dashboard to review it. For a money-reversing operation
that is an audit gap — the attribution may exist on the row but cannot be inspected, so refunds
can't be reviewed or reconciled the way voids can. **Fix direction:** add a refunds view (or fold
refunds into the Voids & Exceptions report as a second section) surfacing
order/cashier/authorizer/reason/amount/time, and confirm the refund handler actually persists
...
panel, the Payment modal, or the receipt screen. Minor, but orders can't be tagged by name (also
why the test agent couldn't label its throwaway orders "ZZ-TEST"), and any workflow that wants
...
`/kds` (the OLD session-based route) cleared the owner's tokens app-wide — every dashboard
route in every tab then redirected to `/login`, stranding the owner mid-task (they could
not void a live order). Likely cause: the old `/kds` raw-fetches `/api/kitchen/tickets`
with no valid auth → 401 → the app's `clearAllTokens()` (`lib/api.ts`) wipes owner + POS +
branch tokens on "session expiry", not just the failing surface. **A3's client rewrite
(`-o`, not yet deployed)** replaces `/kds` with a token-based display that uses a SEPARATE
...
A190, A187 void/refund, A3, A184, A146 email, and A144 came back "absent" because their
code is committed in the repo but not in the deployed dashboard. Those "FAIL/absent"
verdicts are NOT code failures — re-verify after the dashboard is redeployed from current
`dev`. **Next free ID A192.**
**DEV-BOX PASS 2026-09-02 (swiftpos-20c2, browser test).** Generated a KDS token, paired `/kds`,
waited ~30s, returned to the dashboard and navigated (Overview → Reservations, Settings) — the
owner session stayed authenticated, no `/login` redirect in any tab; screenshots either side.
STAYS OPEN pending the same check on `main` post-merge (rule 16) — dev evidence earns the merge,
not the close. (Incidental find opened as A192: `/kds` rendered "all clear" while its ticket
fetch was 401.)

**PROD PASS — CLOSED 2026-09-02 (swiftpos-prod-mype, owner-run retest).** On production, with
...
**Reassessed 2026-08-31.** The test pyramid is largely built and CI is comprehensive
(typecheck ratchet, builds for all three apps, secret scan, schema-drift, server suites
+ migrations against real Postgres, desktop suites). But the top layer runs **nowhere
automatically**: `e2e/` (Playwright, 10 tests across owner/POS/manager/public) and
`tests/suites/` + `tests/runner.mjs` (auth, orders, permissions, restaurant, petrol,
reports, security, stress) both need a reachable target — a dashboard + API + a seeded
test business with owner + staff creds — which CI has none of. So the money-flow E2E
(split bill, void/refund, sync, the tables view) is only ever confirmed by hand, which
keeps one person the single point of verification.

**Constraint:** the API is coupled to hosted Supabase (PostgREST + GoTrue auth); there
is no Supabase-local tooling in the repo. Ephemeral E2E-in-CI therefore means adopting
the Supabase CLI local stack (no secrets) — see `docs/E2E-CI-PLAN.md` for the plan, the
Option A/B decision, and the staged build (from-scratch business+owner seed first).
Owner to pick the approach; then built + validated iteratively via CI runs (not
sandbox-verifiable).

**APPROACH CHOSEN 2026-08-31: Option A (Supabase local stack, no secrets)** — aligned
with the owner's plan to self-host the Supabase *stack* on a VPS (241 `.from()` sites +
GoTrue owner auth make a full rip-out a poor trade for a live money system; self-hosting
keeps the code and the CLI local stack mirrors prod). **STEP 1 STARTED (seed —
iteration 1, unverified):** `e2e/seed/seed-business.mjs` + `npm run seed:business`.
Dependency-free (raw fetch to the GoTrue admin API + `/api/onboarding`): creates a
GoTrue user, onboards a test business+owner, writes OWNER_EMAIL/PASSWORD to `e2e/.env`
for the specs. Portable — the same script bootstraps a fresh self-hosted stack / VPS.
`node --check` clean + the missing-env guard fires; the GoTrue+onboarding calls are NOT
yet run against a live stack (no Docker/Supabase in the delivery sandbox) — validate
locally, then it feeds the `supabase start` CI job (E2E-CI-PLAN steps 2-4).
**Run 1 (2026-08-31):** createUser returned 403 `not_admin` — the `.env`
`SUPABASE_SERVICE_ROLE_KEY` was the anon/publishable key, not service_role (config,
not code; nothing created). Hardened: a `SEED_ALLOW=1` guard (never seed prod by
accident — the target was the live project), a clear wrong-key message, and a clean
exit (`exitCode` not `exit`, sidesteps the A186 Windows teardown crash which also fires
on this script). Re-run needs the real service_role key against a TEST target, not prod.
**Run 2 (2026-08-31):** service_role key fixed → `createUser` 200 ✓; the next call
failed with `fetch failed` — a connection error, the local API not running on :4000
(onboarding target). Seed hardened again: per-endpoint labeled connection errors (so
"fetch failed" now names the URL) + stable/idempotent owner creds (no more orphan auth
users per run). Next: start `apps/server` (or point `API_BASE_URL` at the deployed dev
API), re-run, send output.

### A188 · P2 · FIX BUILT 2026-09-09 · Cloud restaurant POS stacks every table into one box when the branch has no saved floor layout
...
view in `CashierScreen.tsx` absolute-positions each tile at `left: pos_x ?? 40, top:
pos_y ?? 40`; when a branch's tables carry no saved layout (pos_x/pos_y null) every
tile lands at (40,40) and stacks on top of each other. `floorMode` defaults to `true`,
so that collapsed pile is the default view. The grid branch immediately below renders
all tables fine — the desktop `TablesView` avoids this by only using the floor plan
when a table has coords (`hasLayout`), else a grid.

**Fix (dev — OPEN pending browser).** Mirror the desktop: `hasLayout = tables.some(t =>
t.pos_x != null && t.pos_y != null)`; render the floor plan only when `floorMode &&
hasLayout`, else the grid (so a no-layout branch shows every table in a selectable
grid); Floor/Grid toggle hidden until a layout exists. `vite build` exit 0. No data or
...
**PHASE 2b DELIVERED 2026-08-31 (client — dev; OPEN pending browser).** The owner
Orders page now shows a **Void** button on orders inside the 30-min window and a
**Refund** button on completed sales past it (both gated on `orders.void`), each
opening a reason modal that POSTs to the owner self-authorised endpoints (no PIN,
audit trail recorded server-side). `vite build` exit 0 (OrdersPage bundle 8.31 kB).
**All phases of A187 are now built**; it stays OPEN only pending a browser pass:
as owner — void a fresh order (reason required, no PIN) → it drops from sales;
...
**DEV-BOX PASS 2026-09-02 (swiftpos-20c2, browser test via a cashier POS session).** Rang a
fresh order, voided it from Finance → Orders: NO 500, status → Voided, a Void Log row recorded
(order `ORD-MTK2693U-00118K`, cashier Bill, authorizer Beryl Atieno, reason, KES 100, 15:14),
persisted across reload. Refund on a second order: NO 500, order stays Completed with a reversal
line. The successful `voided_by` write is positive evidence migration 96 IS applied on this box —
the FK change the schema health-check can't see. STAYS OPEN pending the same on `main` post-merge
AND confirmation migration 96 is applied on the prod DB there (rule 16); one live void proves it.
Follow-up A193: Refund records reason/authorizer but has no audit-log view.

**PROD PASS — CLOSED 2026-09-02 (swiftpos-prod-mype, owner-run retest).** Void on a real order
returned NO 500, status → Voided, Void Log row recorded (owner name + reason) — the live
confirmation that migration 96 IS applied on the prod DB (the FK the health-check can't see).
Refund on a second order: NO 500, order stays Completed with a `cash -550 (refunded)` reversal
line (e.g. ORD-MTKAOJNI-002269). CLOSED per rule 16. New findings from this run: A195 (refunded
orders show "Completed" with no visual distinction) and A196 (voided/refunded orders still appear
...
longer hides a real one. The crash is now a local-Windows cosmetic annoyance only; the
`run-migration-tests.mjs` teardown fix in the candidates below is still worth doing but
...
**WIRED 2026-08-31.** Turned out mostly built: the rename endpoint
(`PATCH /api/devices/:id/label`, A72) and the per-till telemetry (user / last-seen /
last-sync / app+schema version) already existed and show on FleetPage — the only gap
was a UI to SET the name. Added an inline rename control (hover ✎ → edit → save/Esc) on
the Terminals screen; owners can now label each till (e.g. "Front counter") to tell them
apart and spot dead ones. vite build exit 0, check-api-routes OK (287). Closes on browser
confirm: rename a till → the label persists and distinct tills show distinct names.
...
### A182 · P2 · FIX BUILT 2026-09-07 · MAC-binding so a reinstalled till keeps its name/terminal code (the ROOT of A181's collisions) + session-restore guide
...
### A179 · P1 · FIX BUILT 2026-09-07 · Till-created expenses never sync (non-UUID id 500s the whole cash batch) — FIX BUILT + SELF-HEAL
...
### A168 · P2 · FIX BUILT 2026-09-07 · Order-push 401 refreshed the wrong token for an offline shift
...
### A151 · P1 · FIX BUILT 2026-09-09 · Split Bill (by-item) needs a live confirm — the under-collection is already fixed in code
...
### A154 · P3 · CLOSED 2026-09-02 · Build the admin "DB migrations" panel — `MigrationsPage.tsx` front-end exists, `GET /api/admin/migrations` backend never built
...
**CLOSED 2026-09-02 (built + browser-verified).** Built this session: `GET /api/admin/migrations`
(requireAdmin) reading `schema_migration_runs` + `schema_migrations` → `{ current_version,
total_applied, runs }`; wired the finished-but-unwired `MigrationsPage` into `AdminPortal` (nav +
route, adapting its `req(path)` contract); allowlisted `schema_migration_runs` in
`check-api-schema-drift` (A154 — it is bootstrapped by `migrate.mjs`, not a numbered migration).
No schema change, no prod-migrate. All gates green: server + admin `tsc`, admin build, schema-audit,
check-api-routes, check-schema-drift, check-api-schema-drift + 11/11 self-tests. Owner confirmed
the Migrations panel loads real data in the portal (not the mock's `78_receipt_manage_grant`).
...
### A157 · P2 · FIX BUILT 2026-09-07 · Input-validation schemas written but never wired — `LoginSchema`, `CreateProductSchema`, `UpdateProductSchema`, `CreateCategorySchema`
...
### A141 · P2 · FIX BUILT 2026-09-07 · No bulk ingredient import (must seed opening stock)
...
**RESOLVED-DIAGNOSIS 2026-08-31.** The "no CTA for the owner" is a permission-grant gap,
not a bug: `can('ingredients.manage')` correctly gates the Import/Add CTAs, and the
default catalogue grants owners every permission — but defaults seed AT ONBOARDING, and
B Fastfoods onboarded before `ingredients.manage` existed, so their owner role never got
it (no `*` wildcard either). **Immediate fix (no code):** Settings → Roles → the owner's
role → enable "manage ingredients"; the CTAs appear, then re-test the import. **Systemic
candidate (not built):** a one-time backfill granting newly-added default permissions to
existing roles — helps every permission added since a business onboarded, not just this
one. Flag if wanted (would be its own ID).
...
**DEV-BOX PASS 2026-09-02 (swiftpos-20c2, browser test).** Bulk-tracked 3 untracked products
(Untracked 67→64, Out-of-stock 0→3); set a min threshold and it persisted across a full
navigate-away reload — confirmed BOTH via the dashboard Bar & Packaged Stock view AND the in-POS
Inventory panel (min 12/5 matching what was set from the dashboard). STAYS OPEN pending the same
on `main` post-merge (rule 16). Scope note: this PASS covers bulk-track + threshold only; the two
remaining A144 write-endpoints (transfer approve/complete, direct branch-stock set) are still
UI-unwired.

**PROD PASS — CLOSED 2026-09-02 (swiftpos-prod-mype, owner-run retest).** The delivered slice —
bulk stock-tracking + min-threshold editing — verified on production (tracked products appear in
Inventory; threshold persists across reload), matching the dev box + in-POS Inventory tab earlier.
CLOSED per rule 16 for that scope. The two write-endpoints still without a UI caller (transfer
...
### A146 · P2 · FIX BUILT 2026-09-09 · Notifications & webhook observability — endpoints live, no UI caller
...
**CLOSED 2026-09-02 (browser-verified).** Owner confirmed in the admin portal: the client
Overview's "Web access expiry" row shows and sets `businesses.web_access_expires_at` via
`PATCH /clients/:id/web-access` — the one genuinely-unwired endpoint. The other two (`/audit`,
`/tech/tokens`) were already-wired false positives (query-string call literals). All three admin
endpoints now have working callers.
...
job runs `typecheck-ratchet.mjs server dashboard admin` and the `build` job installs +
builds admin. The "no CI type-check or build" finding is resolved and the errors are no
longer unseen — they're ratcheted (frozen; new ones fail CI). Burning down the
baselined admin errors is ordinary cleanup, not this finding.
...
### A129 · P1 · FIX BUILT 2026-09-07 · Delivery sales silently never sync — cloud `orders.order_type` dropped `delivery` (A128's twin)
...
### A139 · P3 · FIX BUILT 2026-09-09 · Per-branch (franchise) receipt text + hours overriding the business default
...
### A3 · P1 · CLOSED 2026-09-02 · BUG-21 — KDS realtime / RLS
**DIAGNOSED 2026-08-31 (was "unknown, not known-good" — now known).** Three distinct
faults in the `/kds` board:
1. **Auth mismatch (dominant).** `/kds` is a STANDALONE PUBLIC route (App.tsx — outside
   `<ProtectedRoute><DashboardLayout>`): a headless kitchen display with no login, so it
   uses bare `fetch` (no token). But `/api/kitchen/tickets` is behind `requireAuth` (no
   cookie/public fallback) → the display 401s on load, poll, the realtime re-fetch, and
   the status PATCH. KDS can't even show tickets. NEEDS A DECISION on how a headless
   display authenticates: a per-branch KDS read-token; device enrolment like the tills;
   or a branch-scoped public read endpoint.
2. **Realtime publication gap.** `kitchen_tickets` was in NO `supabase_realtime`
   publication, so `postgres_changes` never fired. **FIXED (dev):** migration
   `95_kitchen_tickets_realtime.sql` adds it (idempotent; REPLICA IDENTITY FULL already
   set in 00_baseline). PROD-MIGRATE. Necessary, not sufficient.
3. **Realtime RLS / anon.** The dashboard `supabase` client is `createClient(url,
   anonKey)` with no user token; `kitchen_tickets` has RLS (`owner_all`, branch-scoped).
   If the project enforces RLS on the realtime stream, anon receives nothing even after
   (2). Live-verify after publishing; if events still don't arrive, the subscriber needs
   a valid Supabase-recognised token (ties into the SwiftPOS-JWT-vs-GoTrue question).
...
**DEV-BOX PASS 2026-09-02 (swiftpos-20c2, browser test).** Rang an order for the KDS branch while
watching a paired `/kds` tab: the ticket appeared Pending in ~5–10s; advanced Start → In Progress
→ Ready → Picked Up; the Collected transition survived a full `/kds` reload (server-side, not
optimistic UI); ticket counts moved correctly at each step. The 10s poll fallback is delivering.
STAYS OPEN pending the same on `main` post-merge (rule 16). Related observability defect opened as
A192 (a 401 ticket-fetch renders as "all clear").

**PROD PASS — CLOSED 2026-09-02 (swiftpos-prod-mype, owner-run retest).** On production, a ticket
for a freshly-rung order appeared on `/kds` within ~10s and advanced Start → Ready → Collected,
persisting across reload. CLOSED per rule 16. Separate lifecycle defect found in the same run and
opened as **A196**: voided/refunded orders' tickets are NOT pulled from the board (A3's own
criteria — deliver + advance — passed, so this doesn't block the close).
...
### A69 · P2 · CLOSED 2026-09-02 · Enrolment issuance moves to admin (billable), branch-bound; owner self-provisioning retired
...
**CLOSED 2026-09-02 (browser-verified, Overview path).** Owner confirmed: on a licensed branch,
Enrol till mints single-use, branch-bound code(s) shown inline with the Business ID, per-till
codes, Copy buttons and an expiry (Overview tab). Note: the Branches-tab entry point minted the
code but had no block to display it — opened and fixed as **A198**; that does not hold this close,
as the Overview path is fully working.

---

### A70 · P3 · CLOSED 2026-09-02 · Enrolled-device roster in the admin portal
...
**CLOSED 2026-09-02 (browser-verified).** Owner confirmed the "Enrolled Devices" roster renders
under the client Overview from `GET /clients/:id/devices` — the fleet view (label / role / branch /
status / last-seen / version) populated from real rows, which is the live check this entry was held
for (rule 16).
...
### D17 · P3 · FIX BUILT 2026-09-09 · Desktop build has no dev/prod flavour (icon, appId, userData, update channel)

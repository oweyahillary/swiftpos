...
| Date | Change |
|---|---|
| 2026-08-17 | **A127 closed (admin portal: Branches tab + tills/tech-audit drill-down)** — new Branches tab between Overview and Features. Server: branchId added to admin devices response; new GET /clients/:id/devices/:deviceId/tech-audit (reads tech_audit_log by device, read-only, limit 100). UI: branch list -> click a branch shows its tills (devices filtered by branchId) -> click a till shows its tech audit log (action/tech/detail/when). Reuses existing branch mgmt (create/close/licence/enrol/revoke). Overview branch card left in place (temporary duplicate) pending visual confirmation, then removed in a follow-up. Built on pushed A125+A126 code (their register rows were missing — folded in here). Verified: server tsc clean, admin vite build clean, +3 benign S.input CSS-class errors (68), gates green. NOT bench-testable — click-test drill-down. Delivery: MANIFEST-2026-08-17-r.md. |
| 2026-08-17 | **A126 closed (admin portal Phase 3: glass refresh)** — front-end only, token-level. index.css: Inter + Space Grotesk, aurora body backdrop. AdminPortal C palette -> glass (translucent surface/card, accent #38e1ff + violet #a78bfa); S -> frosted glass (backdrop-blur sidebar/topbar/modal, transparent main, blurred cards); Space Grotesk on wordmark + KPI numbers. Verified: builds clean, 0 new type errors, gates green — VISUAL result needs browser review. Delivery: MANIFEST-2026-08-17-q.md. |
| 2026-08-17 | **A125 closed (purge Stage 2: dry-run preview — non-destructive)** — deletes nothing. Server: explicit PURGE_RETAIN/REVIEW/PURGE table classification + GET /clients/:id/purge-preview (per-table row counts, read-only). UI: Preview purge button on purge-due clients -> RETAIN/REVIEW/PURGE breakdown. branches kept in RETAIN (orders.branch_id references it). Destructive execute deliberately NOT built (accountant/DPO sign-off + staging test required). Verified green. Delivery: MANIFEST-2026-08-17-p.md. |

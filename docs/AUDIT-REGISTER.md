...
| Open | **A: 0 P0 · 19 P1 · 17 P2 · 12 P3 — D: 0 P0 · 2 P1 · 1 P2 · 2 P3** (re-derived from the body by `check-register-consistency`, not hand-counted) |
| Counts | A-P0: — · A-P1: A179 A54 A19 A20 A50 A24 A129 A158 A160 A161 A162 A163 A164 A211 A240 A265 A266 A267 A273 A274 A275 A297 · A-P2: A189 A182 A168 A22 A23 A53 A141 A159 A157 A197 A209 A241 A245 A249 A259 · A-P3: A186 A13 A139 A148 A234 A237 A243 A251 A257 A258 A262 A18 A296 — D-P0: — · D-P1: D3 D4 · D-P2: D18 · D-P3: D9 D10 |
...
### A297 · P1 · OPEN · Ingested (node) orders arrive without their line items — Item Mix / top sellers / Overview item+payment cards read empty while the sale shows in the Orders list
Symptom (target, 0.5.43, B Foods "Mama Ngina", 2026-09-18): the Orders list shows T1--23 (Dine In,
Cash, KES 650, completed, minutes old) yet Item Mix says "No sales yet today" and the Overview KPIs
read 0 / "No payments". Owner reports it worked ~v0.5.30 and asks what changed. Mechanism, from the
source: the Overview and Item Mix aggregate `order_items`/`payments` (getTopProducts / getSalesSummary
in managerReports.ts), while the Orders list (getRecentOrders) reads only the `orders` table — which is
why only the list shows the sale. On a node, `order_items` and `payments` are NOT in `REPLICATED_TABLES`
(nodeIngest.ts); they reach the node only as `_items`/`_payments` piggy-backed on the order
(insertOrderChildren). `fillNodeOutbox` on current code DOES attach both, so an itemless order on the
node is either (a) legacy/old-peer data written before the attach existed (the 08-03 path, 3763946) or
(b) the sending till's own `order_items` was empty at push time. What changed since ~v0.5.30 is the
branch-node ingest/replication path itself (3763946 payments-replication + `_items`; 3dc17b2
node-authority A19/A20/A24; fee91cc bounded replicas): before that a plain till rang orders locally with
their items, so top sellers worked. NOT YET CONFIRMED (needs the till, rules 7/11):
`SELECT COUNT(*) FROM order_items WHERE order_id=(SELECT id FROM orders WHERE order_number='T1--23');`
`SELECT device_role FROM device_config;` and the selling till's build. Bundled robustness gap: the
Overview's main revenue row is the one getSalesSummary sub-query still NOT wrapped in the A293 fail-soft
try/catch, so any throw there blanks all KPIs with no on-screen error — complete A293 + surface an error
state so 0 never masquerades as "no sales". No code shipped for A297 (diagnosis only; a blind change to
the replication/ingest path during the in-flight 0.5.43 release is what rule 13 forbids).
...
### D3 · P1 · FIX BUILT 2026-09-10 · No auto-update — scaffold added, release pipeline outstanding
...
### D18 · P2 · FIX BUILT 2026-09-10 · A tech token pasted into the reveal field is truncated — "not allowing the full string"

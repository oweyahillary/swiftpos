...
The dashboard's menu-upload template (apps/dashboard/src/pages/products/MenuUpload.tsx `downloadTemplate`, built
client-side with SheetJS) already demonstrated the drink-size upgrade ladder (Soda 350ml=0 / 1.25L=+130) but had no
fries upgrade — the gap behind the "add large fries paid" ask. Added a Fries-size ladder example (Regular Fries:
Regular=0 baseline / Large=+60, marked illustrative) and expanded the 4-line Read me into fuller guidance
(matched-by-name/plu_code, sparse column updates, DELETE to clear, lists one-row-per-item with ; not comma, free vs
upgrade ladder needing a 0 baseline, purchased vs central_kitchen cost). Kept the canonical title "SwiftPOS —
Restaurant menu upload template". NOTE for the owner: the client's "Kudo Kudo — Restaurant product import template"
xlsx is a separate/older artifact — the app has always generated the SwiftPOS-titled template, so standardise on the
in-app one. Bench-verified: menu-template.test.mjs 7/7 (source-guard; fries/drink/ladder mutations bite), dashboard
tsc 0 errors. NOT verified (target-only): downloading the template from the live dashboard and re-importing it.
Delivery: docs/MANIFEST-2026-09-21-l.md.
...
### A277 · P2 · OPEN · Receipt omits the Catering/Tourism Levy (CTL) line
...
### A278 · P2 · OPEN · Web→till changes aren't instant — a till restart is needed to pick them up

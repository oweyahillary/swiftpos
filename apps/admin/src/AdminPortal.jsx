...
const TYPE_META = {
  ...
  cafe:          { icon: "☕",  label: "Café",          color: "#a78bfa" },
  retail:        { icon: "🛍️", label: "Retail",        color: "#60a5fa" },
  minimart:      { icon: "🏪", label: "Minimart",      color: "#34d399" },
  parking:       { icon: "🅿️", label: "Parking",       color: "#fbbf24" },
  petrol_station:{ icon: "⛽", label: "Petrol Station",color: "#f43f5e" },
        ...
        <div style={S.card}>
          ...
          {(stats?.recent_signups || []).map(b => (
            <div key={b.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: `1px solid ${C.border}` }}>
              <span style={{ fontSize: 18 }}>{TYPE_META[b.type]?.icon || "🏢"}</span>
              <div style={{ flex: 1 }}>
          ...
          <tbody>
            {health.slice(0, 20).map(b => (
              <tr key={b.id} style={{ cursor: "pointer" }}>
                ...
                <td style={S.td}><span style={{ fontSize: 16 }}>{TYPE_META[b.type]?.icon}</span> <span style={{ fontSize: 12, color: C.muted }}>{TYPE_META[b.type]?.label}</span></td>
          ...
          <tbody>
            {loading ? (
              ...
              <tr key={b.id} style={{ cursor: "pointer" }} onClick={() => onSelectClient(b)}>
                ...
                <td style={S.td}><span style={{ fontSize: 16 }}>{TYPE_META[b.type]?.icon}</span> <span style={{ fontSize: 12, color: TYPE_META[b.type]?.color }}>{TYPE_META[b.type]?.label}</span></td>
                <td style={S.td}><StatusBadge status={b.status} /></td>
  ...
  return (
    <div style={S.content}>
      ...
      <div style={{ marginBottom: 16, padding: "14px 18px", background: hasWebHosting ? "rgba(34,197,94,0.06)" : "rgba(239,68,68,0.06)", border: `1px solid ${hasWebHosting ? "rgba(34,197,94,0.2)" : "rgba(239,68,68,0.2)"}`, borderRadius: 10, display: "flex", alignItems: "center", gap: 14 }}>
        <span style={{ fontSize: 22 }}>{hasWebHosting ? "🌐" : "🔒"}</span>
        <div style={{ flex: 1 }}>
      ...
      <div style={S.content}>
        <div style={{ maxWidth: 540, margin: "0 auto" }}>
          <div style={{ ...S.card, borderColor: "rgba(34,197,94,0.3)", background: "rgba(34,197,94,0.04)", textAlign: "center", padding: "40px 32px" }}>
            <div style={{ fontSize: 48, marginBottom: 16 }}>{TYPE_META[form.businessType]?.icon || "🏢"}</div>

      ...
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => {
            ...
            if (branchId) p.set('branch_id', branchId);
            window.open(`${import.meta.env.VITE_API_URL}/api/reports/export/sales?${p}`);
          }}
...
function FuelSalesTab({ range, branchId, currency }: { range: DateRange; branchId: string; currency: string }) {
  ...
  const fmtL = (n: number) => `${Number(n).toLocaleString('en-KE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} L`;
  const peakHour = (hourlySeries as any[]).reduce((best: any, h: any) => (!best || h.litres > best.litres) ? h : best, null);

  return (
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-3">
        <KpiCard label="Total Revenue" value={fmt(summary.totalRevenue)} accent />
        <KpiCard label="Total Litres"  value={fmtL(summary.totalLitres)} />
          ...
          : (
            <table className="w-full text-sm">
              <thead><tr className="border-b border-gray-200 dark:border-gray-700">
                <Th>Grade</Th><Th right>Litres</Th><Th right>Revenue</Th><Th right>Transactions</Th>
              </tr></thead>
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {grades.map((g: any) => (
                  <tr key={g.product_id}>
                    <td className="py-2 text-gray-800 dark:text-gray-200 font-medium">{g.name}</td>
                    <td className="py-2 text-right tabular-nums">{fmtL(g.litres)}</td>
                    <td className="py-2 text-right tabular-nums">{fmt(g.revenue)}</td>
      ...
      {pumps.length > 0 && (
        <SectionCard title="By Pump">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-gray-200 dark:border-gray-700">
              <Th>Pump</Th><Th right>Litres</Th><Th right>Revenue</Th><Th right>Transactions</Th>
            </tr></thead>
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
              {pumps.map((p: any) => (
                <tr key={p.pump_id}>
                  <td className="py-2 text-gray-800 dark:text-gray-200 font-medium">{p.pump_name}</td>
                  <td className="py-2 text-right tabular-nums">{fmtL(p.litres)}</td>
                  <td className="py-2 text-right tabular-nums">{fmt(p.revenue)}</td>
                  <td className="py-2 text-right tabular-nums text-gray-500">{p.transactions}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <SectionCard title={`Hourly Pattern${peakHour?.litres > 0 ? ` · Peak ${peakHour.hour}:00` : ''}`}>
        {(() => {
          const max = Math.max(...(hourlySeries as any[]).map((h: any) => h.litres), 1);
          const hasData = (hourlySeries as any[]).some((h: any) => h.litres > 0);
          if (!hasData) return <p className="text-sm text-gray-500">No data.</p>;
          return (
            <>
              <div className="flex items-end gap-0.5 h-16">
                {(hourlySeries as any[]).map((h: any) => (
                  <div key={h.hour} className="flex-1 flex flex-col justify-end h-full" title={`${h.hour}:00  ${fmtL(h.litres)}  ${fmt(h.revenue)}`}>
                    <div style={{ height: `${Math.max(4, Math.round((h.litres / max) * 100))}%` }} className="bg-amber-400/80 rounded-sm" />
                  </div>
                ))}
              </div>
              <div className="flex justify-between mt-1">
                {[0, 6, 12, 18, 23].map(h => <span key={h} className="text-[9px] text-gray-500">{h}:00</span>)}
              </div>
            </>
          );
        })()}
...
function WetStockTab({ range, branchId, currency }: { range: DateRange; branchId: string; currency: string }) {
  ...
  const { tanks, deliveries } = data;
  const fmtL = (n: number) => `${Number(n).toLocaleString('en-KE', { maximumFractionDigits: 0 })} L`;
  const fmtC = (n: number) => `${currency} ${Number(n).toLocaleString('en-KE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;

  return (
    <div className="space-y-5">
      <SectionCard title="Tank Levels">
        {tanks.length === 0
          ...
          : (
            <div className="space-y-4">
              {tanks.map((t: any) => {
                ...
                const colour = t.is_low ? 'bg-red-500' : pct < 50 ? 'bg-amber-400' : 'bg-green-500';
                return (
                  <div key={t.id}>
                    <div className="flex items-center justify-between mb-1">
                      <div>
                        <span className="text-sm font-medium text-gray-800 dark:text-gray-200">{t.name}</span>
                        <span className="text-xs text-gray-500 ml-2">{t.product_name}</span>
                        {t.is_low && <span className="ml-2 text-xs font-medium text-red-500">LOW</span>}
                      </div>
                      <span className="text-xs text-gray-500 tabular-nums">{fmtL(t.current_level)} / {fmtL(t.capacity_litres)} ({pct}%)</span>
                    </div>
                    ...
                    <div className="flex justify-between mt-0.5 text-[9px] text-gray-500">
                      <span>Reorder at {fmtL(t.reorder_level)}</span>
                      <span>Capacity {fmtL(t.capacity_litres)}</span>
                    </div>
                  </div>
                );
              })}
      ...
      <SectionCard title="Deliveries in Period">
        {!deliveries?.length
          ? <p className="text-sm text-gray-500">No deliveries recorded in this period.</p>
          : (
            <table className="w-full text-sm">
              <thead><tr className="border-b border-gray-200 dark:border-gray-700">
                <Th>Tank</Th><Th>Grade</Th><Th right>Litres</Th><Th right>Value</Th><Th>Date</Th>
              </tr></thead>
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {(deliveries as any[]).map((d: any, i: number) => (
                  <tr key={i}>
                    <td className="py-2 text-gray-800 dark:text-gray-200">{d.tank_name ?? '—'}</td>
                    <td className="py-2 text-gray-500">{d.product_name ?? '—'}</td>
                    <td className="py-2 text-right tabular-nums">{fmtL(d.litres)}</td>
                    <td className="py-2 text-right tabular-nums">{d.value != null ? fmtC(d.value) : '—'}</td>
                    <td className="py-2 text-gray-500 text-xs">{d.created_at ? new Date(d.created_at).toLocaleDateString('en-KE') : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>

...
const FLOOR_W = 800;
const FLOOR_H = 520;
const TABLE_W = 220;  // large floor tiles
const TABLE_H = 180;
                    ...
                    {tables.map(table => {
                      ...
                      return (
                        <button key={table.id} data-testid="table-tile" data-status={status} data-name={table.name} onClick={() => onTableTap(table)} style={{
                          ...
                          <span style={{ color: occ ? '#f59e0b' : c.text, fontSize: 22, fontWeight: 700, pointerEvents: 'none' }}>
                            {table.name}
                          </span>
                          {order && order.cart.length > 0 ? (
                            <span style={{ color: '#f59e0b', fontSize: 14, fontWeight: 600, marginTop: 5, pointerEvents: 'none' }}>
                              {order.cart.reduce((s, i) => s + i.quantity, 0)} · {timeAgo(order.openedAt)}
                            </span>
                          ) : (
                            <span style={{ color: occ ? '#f59e0b' : c.text, fontSize: 15, opacity: 0.7, marginTop: 5, pointerEvents: 'none' }}>
                              👥{table.capacity}
                            </span>
                          )}
                        </button>

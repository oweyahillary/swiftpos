                ...
                <div style={{ overflowX: 'auto', overflowY: 'auto', flex: 1 }}>
                  <div style={{ position: 'relative', width: FLOOR_W, height: FLOOR_H, background: '#0a0f1a', borderRadius: 12, flexShrink: 0 }}>
                    {tables.map(table => {
                      ...
                      return (
                        <button key={table.id} onClick={() => onTableTap(table)} style={{
            ...
            <div style={s.cartFooter}>
              ...
              {isRestaurant && orderMode === 'order_first' ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  ...
                  <button style={s.chargeBtn} onClick={() => setShowPayment(true)}>
                    Charge {fmt(orderTotal, currency)}
                  </button>
                </div>
              )}
        ...
        <div style={s.overlay}>
          <div style={{ ...s.modal, width: 420, textAlign: 'left' }}>
            ...
            <p style={{ ...s.modalSubtitle, textAlign: 'left', marginBottom: 16 }}>
              Move <strong style={{ color: '#fff' }}>{activeKey && openOrders[activeKey]?.tableName}</strong>'s order to another table
            </p>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    // Create the order tagged as room_charge payment method
                    const result = await posApi.post<{ orderId: string; orderNumber: string }>('/api/orders', {
                      ...
                      guest_name:     roomGuestName.trim() || null,
...
Signature: _______________`;
                    ...
                    if (w) {
                      w.document.write(`<html><head><style>body{font-family:'Courier New',monospace;font-size:12px;padding:16px;white-space:pre;}</style></head><body>${slip}</body></html>`);
                      w.document.close();
                      setTimeout(() => w.print(), 300);
                    }
                    // Clear order

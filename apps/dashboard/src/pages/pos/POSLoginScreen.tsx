...
export default function POSLoginScreen() {
  ...
  useEffect(() => {
    if (sessionStorage.getItem('swiftpos_pos_expired')) {
      sessionStorage.removeItem('swiftpos_pos_expired');
      setError('Your session expired from inactivity. Please sign in again.');
    }
  }, []);

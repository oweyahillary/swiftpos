...
export interface CartActions {
  ...
  selectedVariants:    Record<string, VariantOption & { groupId?: string; groupName?: string }>;
...
export function useCart(): CartActions {
  ...
  const [selectedVariants, setSelectedVariants] = useState<Record<string, VariantOption & { groupId?: string; groupName?: string }>>({});
  ...
  const toggleVariantOption = useCallback((group: VariantGroup, option: VariantOption) => {
    setSelectedVariants(prev => ({ ...prev, [group.id]: { ...option, groupId: group.id, groupName: group.name } }));
  }, []);

 ...
 *
 * NOTE: SCOPE §6 also asks for a RECEIPT preview. Receipt-logo printing (monochrome `logo_receipt`)
...
export default function BrandingTab() {
  ...
  useEffect(() => {
    api.get<{ accent_hex: string | null; logo_png: string | null } | null>('/api/business/branding')
      .then((b) => { setAccentHex(b?.accent_hex ?? ''); setLogoPng(b?.logo_png ?? null); })
      .catch(() => {})
  ...
  const onPick = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try { const { dataUri, warning } = await prepareLogo(f); setLogoPng(dataUri); if (warning) setWarn(warning); }
  ...
  const clearAll = async () => {
    setBusy(true); setMsg('');
    try {
      await api.put('/api/business/branding', { accent_hex: null, logo_png: null });
      setAccentHex(''); setLogoPng(null); setWarn('');
    ...
    <div className="p-6 max-w-3xl">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div>
          ...
          {logoPng && <button onClick={() => setLogoPng(null)} className="text-xs text-gray-500 underline mt-1">Remove logo</button>}

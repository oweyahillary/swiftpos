...
async function recomputeOrderTotals(
  ...
  if (lineProductIds.length) {
    ...
    const { data: vgroups } = await supabase
      .from('variant_groups')
      .select('name, product_id, variant_options ( name, price_adjustment )')
      .in('product_id', lineProductIds);
    (vgroups ?? [] as Array<{ name: string; product_id: string; variant_options: Array<{ name: string; price_adjustment: string }> }>).forEach(g =>
  ...
  for (const item of items) {
    ...
    if (pid) {
      ...
      let unit = basePriceMap.get(pid)!;
      for (const v of (item.selectedVariants ?? [])) {
        const adj = variantAdjMap.get(vKey(pid, v.groupName, v.optionName));
        if (adj === undefined) return { ok: false, status: 400, error: `Unknown variant: ${v.groupName} / ${v.optionName}` };

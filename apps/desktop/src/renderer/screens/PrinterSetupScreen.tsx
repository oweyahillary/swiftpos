...
export default function PrinterSetupScreen({ stations }: { stations: Station[] }) {
  ...
  // A43 + local override. The exclusion box is editable on every till. What is
  // shown is the EFFECTIVE list: a per-terminal override if one is set, else the
  // business-wide default synced from the web dashboard. Editing here sets the
  // override, which wins and is never overwritten by a sync — "local is final".
  // "Reset to cloud default" drops the override and follows the dashboard again.
  ...
  // Saved on blur, not per keystroke — a list somebody types, not a toggle, the
  // same choice the dashboard editor makes. One term per line, so the till never
  // has to guess at a separator. Any save sets the override → source becomes local.
        ...
        {station ? (
          <>
            ...
            <section className="flex-1 p-6 overflow-auto min-w-0">
              ...
              {/* Kitchen exclusions. Editable on every till: the box shows the
                  effective list — a local override if set, else the business
                  default synced from the dashboard — and editing sets the
                  override, which wins and survives every sync. */}
              <div className="mt-6 max-w-md">
                ...
                <textarea
                  ...
                  className="w-full bg-gray-900 border border-gray-700 rounded-xl px-3 py-2
                             text-gray-200 text-sm font-mono h-20 resize-y
                ...
                <div className="flex items-start justify-between gap-3 mt-1">
                  <p className="text-xs text-gray-500">
                    {exclSource === 'local' ? (
                      <>
                        This terminal&rsquo;s own list, on top of the built-in drinks rule.
                        It overrides the business default and is kept through every sync.
                        Saved when you click away.
                      </>
                    ) : (
                      <>
                        The business default from the web dashboard, on top of the built-in
                        rule. Edit here to set a list just for this terminal; saved when you
                        click away.
                      </>
                    )}
                  </p>
                  {exclSource === 'local' && (
                    <button
                      ...
                      className="shrink-0 text-xs text-gray-400 underline underline-offset-2
                                 hover:text-gray-200"
                      title={cloudExclusions.length
                        ? `Dashboard: ${cloudExclusions.join(', ')}`
                        : 'Dashboard has no exclusions set'}
                    >

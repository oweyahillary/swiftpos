...
interface Props {
  product: any;
  currency: string;
  onConfirm: (selectedVariants: any[], selectedModifiers: any[], unitPrice: number, lineTotal: number) => void;
  onClose: () => void;
}
...
export default function VariantModal({ product, currency, onConfirm, onClose }: Props) {
  ...
  useEffect(() => {
    const fetches: Promise<any>[] = [
      product.has_variants ? posApi.pos.getVariants(product.id) : Promise.resolve([]),
      product.has_modifiers ? posApi.pos.getModifiers(product.id) : Promise.resolve([]),
    ];
  ...
  const selectVariant = (group: any, optionId: string) => {
    const option = group.variant_options.find((o: any) => o.id === optionId);
    if (!option) return;
    setSelectedVariants(prev => ({
      ...prev,
      [group.id]: { groupId: group.id, groupName: group.name, optionId: option.id, optionName: option.name, priceAdjustment: option.price_adjustment },
    }));
  };

  const toggleModifier = (group: any, optionId: string) => {
    ...
    setSelectedModifiers(prev => {
      ...
      return { ...prev, [key]: { groupId: group.id, groupName: group.name, optionId: option.id, optionName: option.name, price: option.price } };
    });
  };
  ...
  const handleConfirm = () => {
    ...
    for (const group of variantGroups) {
      if (group.required && !selectedVariants[group.id]) { setError(`Please select a ${group.name}`); return; }
    }
    for (const group of modifierGroups) {
      if (group.min_select > 0) {
        const selected = Object.values(selectedModifiers).filter((m: any) => m.groupId === group.id).length;
        if (selected < group.min_select) { setError(`Please select at least ${group.min_select} option(s) for ${group.name}`); return; }
      }
    }
  ...
  const previewUnit = computeUnitPrice(product, Object.values(selectedVariants));
  const previewTotal = computeLineTotal(previewUnit, 1, Object.values(selectedModifiers));
